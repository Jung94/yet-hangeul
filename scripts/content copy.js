...
let En_type = default_En_type; // 영문 자판 종류 (ohiChange 함수로 바꿈)
let Ko_type = default_Ko_type; // 한글 자판 종류 (ohiChange 함수로 바꿈)
let ohi_KBD_type = default_ohi_KBD_type; // 기준 자판 종류 (QWERTY/QWERTZ/AZERTY, ohiChange_KBD_type 함수로 바꿈)
let ohi_KE = default_ohi_KE; // 한글·영문 상태 (Ko: 한글, En: 영문) (ohiChange_KE 함수로 바꿈)
...
function initialize_options() {
  ...
  var default_enable_old_hangeul_input = 1;
...
function show_options() {
  ...
  if (opts) {
    ...
    if (!opt) {
      opt = appendChild(opts, 'div', 'option', 'text_copy_button', '<button onclick="copyToClipboard(document.getElementById(\'inputText\'))">복사하기</button>');
    }
...
function show_keyboard_layout_info() {
  const KE = ohi_KE;
  const kbd = ohi_KBD_type == 'QWERTY' ? '' : ':' + ohi_KBD_type;
  const type_name = typeof current_layout_info.type_name != 'undefined' ? current_layout_info.type_name : '';
  let name = '', link = '', keyboardLayoutInfo = document.getElementById('keyboardLayoutInfo');
  console.log(KE, kbd, type_name, name)
